...
// Helper: Build common email wrapper
const buildEmailHtml = (title, contentHtml) => {
  return `
    <table style='border-collapse: collapse; width: 100%; max-width: 600px; margin: 0 auto; font-family: Verdana, Arial, sans-serif;'>
      ...
      <tr>
        <td style='background-color:#ffffff; padding:25px 20px; text-align:center;'>
          ...
          <p style='margin:10px 0 0 0; font-size:15px; font-weight:bold; color:#333; letter-spacing:1px;'>DIGITAL BUSINESS PEOPLE</p>
        </td>
      </tr>
      <!-- Content -->
      <tr>
                ...
                <table style='width:100%;'>
                  <tr>
                    ...
                    <td style='padding-left:15px;'>
                      <p style='margin:0; font-size:15px; font-weight:bold; color:#333;'>${title}</p>
                    </td>
                  </tr>
                </table>
              </td>
...
async function generateCertificatePDF(userName, courseName, authorName, issueDate, expiryDate) {
 
  const rootDir = path.resolve(__dirname, '..', '..'); // go to backend root
  // 1. Read background.png → base64 data-URI (same as certificate.html inline approach)
  const bgPath = path.join(rootDir, 'backend', 'utils', 'assets', 'background.png');
  console.log('BG Path:', bgPath);
  console.log('BG exists:', fs.existsSync(bgPath));
  let bgDataUri = '';
  try {
    const bgBytes = fs.readFileSync(bgPath);
    bgDataUri = 'data:image/png;base64,' + bgBytes.toString('base64');
  } catch (e) {
    console.warn('[emailService] background.png not found at', bgPath, '— using plain background');
  }

  const logoPath = path.join(rootDir, 'backend', 'utils', 'assets', 'logo.png');
...
try {
  ...
  console.warn('[emailService] logo.png not found at', logoPath);
}
  ...
  const certNumber = `Certificate No. DBP-${new Date().getFullYear()}-${suffix}`;
 
  // 3. Build HTML — this is the EXACT same structure as certificate.html,
  //    with dynamic values injected and the background/logo both inline base64.
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cert-border-inner {
    ...
    border: 0.75px solid rgba(0,102,179,0.12);
    border-radius: 2px;
    pointer-events: none;
  }
 
  .corner { position: absolute; width: 48px; height: 48px; opacity: 0.55; }
  ...
  .logo-wrap img { height: 52px; width: auto; display: block; margin: 0 auto; }
 
  .rule {
    width: 100%;
    max-width: 480px;
    height: 1px;
    background: linear-gradient(90deg, transparent, rgba(0,102,179,0.4), rgba(200,60,100,0.3), transparent);
    margin: 0 auto 28px;
  }
  ...
  .divider-line { width: 110px; height: 1px; background: linear-gradient(90deg, transparent, rgba(0,102,179,0.5)); }
  .divider-line.right { background: linear-gradient(90deg, rgba(0,102,179,0.5), transparent); }
  .divider-diamond { width: 8px; height: 8px; background: #0066B3; transform: rotate(45deg); opacity: 0.7; }
  ...
  .cert-name::after {
    ...
    width: 70%; height: 1px;
    background: linear-gradient(90deg, transparent, rgba(0,102,179,0.35), transparent);
  }
  ...
  .sig-line { width: 130px; height: 1px; background: rgba(0,102,179,0.3); margin-bottom: 6px; }
  .sig-name {
    font-family: 'Montserrat', sans-serif;
    font-size: 9.5px;
    font-weight: 600;
...
<div class="cert-wrapper">
  <div class="cert-page">
    <img class="cert-bg-img" src="${bgDataUri}" />
    <div class="cert-border-outer"></div>
    <div class="cert-border-inner"></div>
 
    <div class="corner tl">
      <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M4 44 L4 4 L44 4" stroke="#0066B3" stroke-width="1.5" fill="none"/>
        <path d="M4 30 L4 4 L30 4" stroke="#0066B3" stroke-width="0.75" fill="none"/>
        <circle cx="4" cy="4" r="3" fill="#0066B3" opacity="0.6"/>
      </svg>
    </div>
    <div class="corner tr">
      <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <img src="${logoDataUri}" />
      ...
      <div class="cert-headline">Excellence</div>
      ...
      <div class="cert-desc">
        has successfully completed the learning course and fulfilled
        all the requirements to be awarded this certificate in
      </div>
      ...
      <div class="signatures">
        <div class="sig-block">
          ...
          <div class="sig-name">Ashish Bhalla</div>
          <div class="sig-title">Chief Executive Officer</div>
        </div>
        ${authorName ? `<div class="sig-block">
          <div class="sig-line"></div>
          <div class="sig-name">${authorName}</div>
          <div class="sig-title">Publisher</div>
        </div>` : ''}
      </div>
 
    </div>
 
    <div class="cert-id-topleft">${certNumber}</div>
    <div class="cert-date">Issued: ${issueDate}</div>
    <div class="cert-expiry">Valid until: ${expiryDate}</div>
 
    <svg class="cert-seal" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="32" cy="32" r="30" stroke="#0066B3" stroke-width="1.5"/>
      <circle cx="32" cy="32" r="26" stroke="#0066B3" stroke-width="0.75"/>
      <circle cx="32" cy="32" r="5" fill="#0066B3"/>
      <text x="32" y="14" text-anchor="middle" font-family="Montserrat,sans-serif" font-size="4.5" font-weight="600" letter-spacing="2" fill="#0066B3">DBP</text>
    </svg>
  ...
  // 4. Launch Puppeteer — no file:// flags needed since everything is inline base64
...
const buildWelcomeEmailHtml = (userName, userEmail) => {
  ...
  `;
  return buildEmailHtml('Welcome to Digital Business People', content);
};
...
const buildNewUserNotificationHtml = (userName, userEmail, createdAt) => {
  ...
  `;
  return buildEmailHtml('New User Registration', content);
};

// Acknowledgement email (course completion)
const buildAcknowledgementHtml = (userName, topicName, score, total) => {
  ...
  `;
  return buildEmailHtml('Course Completion Acknowledgement', content);
};

// Promotion to author email
const buildPromotionToAuthorHtml = (userName) => {
  const content = `
    <p style='font-size:12px; color:#333;'>Dear <b>${userName}</b>,</p>
    <p style='font-size:12px; color:#555;'>Congratulations! You have been promoted to the role of <b>Author</b> on the Digital Business People e‑learning platform.</p>
    <p style='font-size:12px; color:#555; line-height:1.8;'>
      As an author, you can now:
    </p>
    <ul style='font-size:12px; color:#555; margin-left: 20px;'>
      <li>Create new courses with modules and quiz questions</li>
      <li>Edit and delete your own courses</li>
      <li>Help shape the learning experience for other users</li>
    </ul>
    <p style='font-size:12px; color:#555; line-height:1.8;'>
      You can access your author dashboard by logging in and visiting the <b>Author Dashboard</b>.
    </p>
    <p style='font-size:12px; color:#555;'>
      Thank you for your valuable contributions.<br/>
      Digital Business People Team
    </p>
  `;
  return buildEmailHtml('You’ve Been Promoted to Author!', content);
};

// Quiz pass email
const buildQuizPassEmailHtml = (userName, topicName, score, total) => {
  const content = `
    <p style='font-size:12px;'>Dear <b>${userName}</b>,</p>
    <p style='font-size:12px;'>Congratulations! You have passed the quiz for <b>${topicName}</b>.</p>
    <p style='font-size:12px;'>Your score: <b>${score}/${total}</b> (${Math.round((score/total)*100)}%).</p>
    <p style='font-size:12px;'>This course is now marked as completed.</p>
    <p style='font-size:12px;'>Keep learning!</p>
    <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
  `;
  return buildEmailHtml('Quiz Result – Passed', content);
};

// Quiz fail email
const buildQuizFailEmailHtml = (userName, topicName, score, total, attemptsRemaining) => {
  const content = `
    <p style='font-size:12px;'>Dear <b>${userName}</b>,</p>
    <p style='font-size:12px;'>You did not pass the quiz for <b>${topicName}</b>.</p>
    <p style='font-size:12px;'>Your score: <b>${score}/${total}</b> (${Math.round((score/total)*100)}%).</p>
    <p style='font-size:12px;'>You have <b>${attemptsRemaining}</b> attempt(s) remaining. Please note that after each failed attempt, you must wait <b>2 hours</b> before retrying.</p>
    <p style='font-size:12px;'>We encourage you to review the course material and try again.</p>
    <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
  `;
  return buildEmailHtml('Quiz Result – Not Passed', content);
};

// Send functions (unchanged except using new builders)
...
// ---------- Corrected sendAcknowledgementEmail ----------
const sendAcknowledgementEmail = async (userName, userEmail, course, score, total, authorName) => {
  try {
    // Validate course object
    if (!course || !course.title) {
      const errorMsg = `Invalid course object: ${JSON.stringify(course)}`;
      console.error(errorMsg);
      throw new Error(errorMsg);
    }

    const issueDate = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    const expiryDate = 'March 31, 2027';
    const pdfBuffer = await generateCertificatePDF(userName, course.title, authorName || '', issueDate, expiryDate);
    const attachments = [{
      filename: `Certificate_${course.title.replace(/\s/g, '_')}.pdf`,
      content: pdfBuffer,
      contentType: 'application/pdf',
    }];
    const content = `
      <p style='font-size:12px;'>Dear <b>${userName}</b>,</p>
      <p style='font-size:12px;'>Congratulations on successfully completing the course: <b>${course.title}</b>!</p>
      <p style='font-size:12px;'>Your final score: <b>${score}/${total}</b> (${Math.round((score/total)*100)}%).</p>
      <p style='font-size:12px;'>Please find attached your certificate of achievement.</p>
      <p style='font-size:12px;'>Keep learning!</p>
      <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
    `;
    const html = buildEmailHtml('Course Completion Acknowledgement', content);
    await sendEmail(userEmail, `Course Completion Acknowledgement: ${course.title}`, html, attachments);
    return true;
  } catch (error) {
...
const sendPromotionToAuthorEmail = async (userName, userEmail) => {
  const html = buildPromotionToAuthorHtml(userName);
  return sendEmail(userEmail, 'Congratulations! You are now an Author', html);
};

const sendQuizPassEmail = async (userName, userEmail, topicName, score, total) => {
  const html = buildQuizPassEmailHtml(userName, topicName, score, total);
  return sendEmail(userEmail, `Quiz Result: Passed - ${topicName}`, html);
};

const sendQuizFailEmail = async (userName, userEmail, topicName, score, total, attemptsRemaining) => {
  const html = buildQuizFailEmailHtml(userName, topicName, score, total, attemptsRemaining);
  return sendEmail(userEmail, `Quiz Result: Failed - ${topicName}`, html);
};


// ---------- Password reset OTP (using common template) ----------
const buildPasswordResetOtpHtml = (otp) => {
  const content = `
    <p style='font-size:12px;'>You requested to reset your password. Use the following OTP to proceed:</p>
    <p style='font-size:24px; font-weight:bold; text-align:center; letter-spacing:4px;'>${otp}</p>
    <p style='font-size:12px;'>This OTP is valid for 15 minutes.</p>
    <p style='font-size:12px;'>If you did not request this, please ignore this email.</p>
    <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
  `;
  return buildEmailHtml('Password Reset Request', content);
};

const sendPasswordResetOTP = async (userEmail, otp) => {
  const html = buildPasswordResetOtpHtml(otp);
  return sendEmail(userEmail, 'Password Reset OTP', html);
};

// Generate badge HTML with dynamic course name (and optionally user name)
function buildBadgeHtml(courseName, userName = '') {
  
  const template = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>DBP Badge</title>
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .ring-outer {
    ...
    background: conic-gradient(from 0deg, #FD0039, #6C3A7F, #0066B3, #00C2F6, #FD0039) border-box;
  ...
  .dot:nth-child(2) { animation-delay: 0.3s; background: #6C3A7F; }
  .dot:nth-child(3) { animation-delay: 0.6s; background: #FD0039; }
  .course-label {
...
<div class="badge">
  ...
  <div class="hex-body">
    <svg class="hex-svg" viewBox="0 0 220 220" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        <linearGradient id="gRim" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%"   stop-color="#FD0039"/>
          <stop offset="33%"  stop-color="#6C3A7F"/>
          <stop offset="66%"  stop-color="#0066B3"/>
          <stop offset="100%" stop-color="#00C2F6"/>
        </linearGradient>
        <linearGradient id="gInner" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%"   stop-color="#0066B3" stop-opacity="0.10"/>
          <stop offset="100%" stop-color="#00C2F6" stop-opacity="0.03"/>
        </linearGradient>
        <clipPath id="hexClip">
          <polygon points="110,8 202,58 202,162 110,212 18,162 18,58"/>
        </clipPath>
      </defs>
      <polygon points="110,4 206,55 206,165 110,216 14,165 14,55"
        fill="none" stroke="url(#gRim)" stroke-width="2.5" opacity="0.6"/>
      <polygon points="110,8 202,58 202,162 110,212 18,162 18,58"
        fill="url(#g1)" stroke="#d0dce8" stroke-width="1"/>
      <g clip-path="url(#hexClip)" stroke="#0066B3" stroke-width="0.5" opacity="0.08">
        <line x1="18" y1="80"  x2="202" y2="80"/>
        <line x1="18" y1="110" x2="202" y2="110"/>
        <line x1="18" y1="140" x2="202" y2="140"/>
        <line x1="60" y1="8"   x2="60"  y2="212"/>
        <line x1="110" y1="8"  x2="110" y2="212"/>
        <line x1="160" y1="8"  x2="160" y2="212"/>
      </g>
      <polygon points="110,18 192,65 192,155 110,202 28,155 28,65"
        fill="url(#gInner)" stroke="#0066B3" stroke-width="0.8" opacity="0.4"/>
      <circle cx="110" cy="11"  r="3" fill="#00C2F6"/>
      <circle cx="199" cy="59"  r="3" fill="#0066B3"/>
      <circle cx="199" cy="161" r="3" fill="#6C3A7F"/>
      <circle cx="110" cy="209" r="3" fill="#FD0039"/>
      <circle cx="21"  cy="161" r="3" fill="#0066B3"/>
      <circle cx="21"  cy="59"  r="3" fill="#00C2F6"/>
    </svg>
    <div class="hex-content">
      <div class="logo-box"><?xml version="1.0" encoding="utf-8"?>
...
<g>
	...
	<g>
      ...
      <div class="course-label">${courseName}</div>
    </div>
  </div>
</div>
</body>
</html>`;
  // Replace the course label (safe because we trust courseName)
  return template.replace(/<div class="course-label">.*<\/div>/, `<div class="course-label">${escapeHtml(courseName)}</div>`);
}

/**
 * Generate a PNG image of the badge using Puppeteer
 * @param {string} courseName
 * @param {string} userName (optional)
 * @returns {Promise<Buffer>} PNG buffer
 */
async function generateBadgeImage(courseName, userName = '') {
  const html = buildBadgeHtml(courseName, userName);
  ...
  try {
    ...
    if (!badgeElement) throw new Error('Badge element not found');

    const screenshot = await badgeElement.screenshot({
      type: 'png',
      omitBackground: true,   // ✅ makes corners transparent
    });
...
/**
 * Send badge email with attached PNG image
 * @param {string} userName 
 * @param {string} userEmail 
 * @param {string} courseName 
 * @returns {Promise<boolean>}
 */
const sendBadgeEmail = async (userName, userEmail, courseName) => {
  try {
    const badgeImageBuffer = await generateBadgeImage(courseName, userName);
    const attachments = [{
      filename: `Badge_${courseName.replace(/\s/g, '_')}.png`,
      content: badgeImageBuffer,
      contentType: 'image/png',
    }];
    const content = `
      <p style='font-size:12px;'>Dear <b>${userName}</b>,</p>
      <p style='font-size:12px;'>Congratulations on successfully completing the course: <b>${courseName}</b>!</p>
      <p style='font-size:12px;'>Please find attached your achievement badge.</p>
      <p style='font-size:12px;'>Keep learning!</p>
      <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
    `;
    const html = buildEmailHtml('Your Achievement Badge', content);
    await sendEmail(userEmail, `Your Badge for ${courseName}`, html, attachments);
    return true;
  } catch (error) {
    console.error('Failed to generate/send badge:', error);
    return false;
  }
};

const buildCourseAssignmentHtml = (userName, courseNames, customMessage = '') => {
  const messageSection = customMessage ? `<p style='font-size:12px;'><strong>Message from admin:</strong> ${customMessage}</p>` : '';
  return `
    <p style='font-size:12px;'>Dear <b>${userName}</b>,</p>
    <p style='font-size:12px;'>You have been assigned the following course(s):</p>
    <p style='font-size:12px;'><b>${courseNames}</b></p>
    ${messageSection}
    <p style='font-size:12px;'>You can access them from your dashboard.</p>
    <p style='font-size:12px;'>Happy learning!</p>
    <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
  `;
};

const sendCourseAssignmentEmail = async (userName, userEmail, courseNames, customMessage = '') => {
  const content = buildCourseAssignmentHtml(userName, courseNames, customMessage);
  const html = buildEmailHtml('Course Assignment', content);
  return sendEmail(userEmail, 'New Course Assignment', html);
};

// Course created email (sent to author)
const buildCourseCreatedHtml = (authorName, courseTitle, courseId, estimatedTime) => {
  const content = `
    <p style='font-size:12px;'>Dear <b>${authorName}</b>,</p>
    <p style='font-size:12px;'>Your course <b>“${courseTitle}”</b> has been successfully created on the Digital Business People e‑learning platform.</p>
    <p style='font-size:12px;'><strong>Course ID:</strong> ${courseId}</p>
    <p style='font-size:12px;'><strong>Estimated Time:</strong> ${estimatedTime} minutes</p>
    <p style='font-size:12px;'>You can now edit or add more content from your author dashboard.</p>
    <p style='font-size:12px;'>Thank you for contributing to our learning community!</p>
    <p style='font-size:12px;'>Best regards,<br/>Digital Business People Team</p>
  `;
  return buildEmailHtml('Course Created Successfully', content);
};

const sendCourseCreatedEmail = async (authorName, authorEmail, courseTitle, courseId, estimatedTime) => {
  const html = buildCourseCreatedHtml(authorName, courseTitle, courseId, estimatedTime);
  return sendEmail(authorEmail, `Your course "${courseTitle}" has been created`, html);
};

// Course deleted email (sent to author)
const buildCourseDeletedHtml = (authorName, courseTitle, courseId) => {
  const content = `
    <p style='font-size:12px;'>Dear <b>${authorName}</b>,</p>
    <p style='font-size:12px;'>Your course <b>“${courseTitle}”</b> (ID: ${courseId}) has been successfully deleted from the Digital Business People e‑learning platform.</p>
    <p style='font-size:12px;'>If this was done by mistake, please contact the support team.</p>
    <p style='font-size:12px;'>Thank you for using our platform.</p>
    <p style='font-size:12px;'>Best regards,<br/>Digital Business People Team</p>
  `;
  return buildEmailHtml('Course Deleted', content);
};

const sendCourseDeletedEmail = async (authorName, authorEmail, courseTitle, courseId) => {
  const html = buildCourseDeletedHtml(authorName, courseTitle, courseId);
  return sendEmail(authorEmail, `Your course "${courseTitle}" has been deleted`, html);
};

// Password reset success email
const buildPasswordResetSuccessHtml = (userName) => {
  const content = `
    <p style='font-size:12px;'>Dear <b>${userName}</b>,</p>
    <p style='font-size:12px;'>Your password has been successfully reset.</p>
    <p style='font-size:12px;'>If you did not perform this action, please contact our support team immediately.</p>
    <p style='font-size:12px;'>You can now log in with your new password.</p>
    <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
  `;
  return buildEmailHtml('Password Reset Successful', content);
};

const sendPasswordResetSuccessEmail = async (userName, userEmail) => {
  const html = buildPasswordResetSuccessHtml(userName);
  return sendEmail(userEmail, 'Your password has been reset', html);
};

const buildReminderHtml = (userName, courseTitle, deadlineDate) => {
  const content = `
    <p style='font-size:12px;'>Dear <b>${userName}</b>,</p>
    <p style='font-size:12px;'>This is a reminder that you have not yet completed the course <b>“${courseTitle}”</b>.</p>
    <p style='font-size:12px;'>The deadline was <b>${new Date(deadlineDate).toDateString()}</b>. Please complete it as soon as possible.</p>
    <p style='font-size:12px;'>You can access your assigned courses from your dashboard.</p>
    <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
  `;
  return buildEmailHtml('Course Completion Reminder', content);
};

const sendReminderEmail = async (userName, userEmail, courseTitle, deadlineDate) => {
  const html = buildReminderHtml(userName, courseTitle, deadlineDate);
  return sendEmail(userEmail, `Reminder: Complete "${courseTitle}"`, html);
};

const buildAuthorReminderHtml = (authorName, userName, courseTitle) => {
  const content = `
    <p style='font-size:12px;'>Dear <b>${authorName}</b>,</p>
    <p style='font-size:12px;'>This is a notification that the user <b>${userName}</b> has not completed the course <b>“${courseTitle}”</b> within the assigned deadline.</p>
    <p style='font-size:12px;'>Please contact the user and follow up as needed.</p>
    <p style='font-size:12px;'>Thank you,<br/>Digital Business People Team</p>
  `;
  return buildEmailHtml('Course Completion Overdue – Notification', content);
};

const sendAuthorReminderNotification = async (authorName, authorEmail, userName, courseTitle) => {
  const html = buildAuthorReminderHtml(authorName, userName, courseTitle);
  return sendEmail(authorEmail, `Action Required: ${userName} has not completed "${courseTitle}"`, html);
};

// Export all functions (including the new/updated ones)
module.exports = { 
  sendWelcomeEmail, 
  sendNewUserNotification, 
  sendAcknowledgementEmail, 
  sendPromotionToAuthorEmail,
  sendQuizPassEmail,
  sendQuizFailEmail,
  sendPasswordResetOTP,
  sendBadgeEmail,      // <-- add this
  sendCourseAssignmentEmail,
  sendCourseCreatedEmail,
  sendCourseDeletedEmail,
  sendPasswordResetSuccessEmail,
  sendReminderEmail,
  sendAuthorReminderNotification 
};